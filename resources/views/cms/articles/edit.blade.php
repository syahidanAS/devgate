...
@section('title', 'Edit Artikel — DevGate')
...
<div class="flex items-center gap-2 text-xs font-semibold text-slate-400">
    ...
    <i class="fa-solid fa-chevron-right text-[8px]"></i>
    <span class="text-white">Edit Artikel</span>
</div>
@endsection
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-800/60 pb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight flex items-center gap-3">
                <i class="fa-solid fa-pen-nib text-indigo-500"></i> Edit Artikel
            </h1>
            <p class="text-slate-400 text-xs mt-1">Ubah isi konten, jadwalkan ulang publikasi, atau perbarui produk komponen terkait.</p>
        </div>
        <a 
    ...
    <!-- Form -->
    <form id="article-form" action="{{ route('cms.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 shadow-xl space-y-5">
                    ...
                    <!-- Article Title -->
                    <div class="space-y-2">
                        <label for="title" class="block text-xs font-bold text-slate-455 uppercase tracking-wider">Judul Artikel</label>
                        <input 
                            type="text" 
                            name="title" 
                            id="title" 
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea 
                            ...
                            id="excerpt" 
                            rows="2" 
                            placeholder="Tulis ringkasan singkat artikel dalam 1-2 kalimat..." 
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="hidden" name="body" id="body_input" value="{{ old('body', $article->body) }}">
                ...
                <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 shadow-xl space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-800">
                        @php
                            $relatedProductIds = $article->relatedProducts->pluck('id')->toArray();
                        @endphp
                        @forelse($products as $product)
                            <label class="flex items-center gap-2.5 p-3 rounded-xl border border-slate-900 bg-slate-950/50 hover:bg-slate-900/40 cursor-pointer select-none transition-all">
                                <input 
                                    ...
                                    {{ (old('products') && in_array($product->id, old('products'))) || (!old('products') && in_array($product->id, $relatedProductIds)) ? 'checked' : '' }}
                                >
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 shadow-xl space-y-5">
                    ...
                    <div class="space-y-2" x-data="{ status: '{{ old('status', $article->status) }}' }">
                    ...
                    <!-- Category Selector -->
                    <div class="space-y-2">
                        <label for="category_id" class="block text-xs font-bold text-slate-455 uppercase tracking-wider">Kategori Artikel</label>
                        <select 
                            name="category_id" 
                            id="category_id" 
                            class="w-full rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-2.5 text-xs text-white focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-slate-455 uppercase tracking-wider">Thumbnail Utama</label>
                        ...
                        >
                            ...
                            <template x-if="preview">
                                <div class="relative rounded-xl overflow-hidden max-h-40 border border-slate-800">
                                    ...
                                    <button 
                                        type="button" 
                                        @click.stop="preview = ''; $refs.thumbnail_file.value = ''"
                            ...
                            <template x-if="!preview">
                                <div class="py-6 space-y-2 text-slate-500">
                                    ...
                                    <span class="text-xs font-bold block text-slate-450">Ubah Thumbnail</span>
                            ...
                            @foreach($tags as $tag)
                                <label class="relative flex items-center select-none cursor-pointer">
                                    <input 
                                        ...
                                        value="{{ $tag->id }}" 
                                        class="peer hidden"
                                        {{ (old('tags') && in_array($tag->id, old('tags'))) || (!old('tags') && in_array($tag->id, $articleTagIds)) ? 'checked' : '' }}
                                    >
                    ...
                    <div class="space-y-3 pt-3 border-t border-slate-900">
                        ...
                        <label class="flex items-center justify-between cursor-pointer select-none">
                            ...
                            <input 
                                ...
                                name="allow_comments" 
                                value="1" 
                                class="rounded border-slate-800 bg-slate-955 text-indigo-500 focus:ring-indigo-500 h-4.5 w-4.5 shadow-sm"
...
<script>
    $(document).ready(function() {
        ...
        const quill = new Quill('#editor', {
            theme: 'snow',
            placeholder: 'Tulis isi konten artikel teknologi / tutorial di sini...',
            modules: {
                toolbar: [
                    [{ 'header': [2, 3, false] }],
        ...
        // Set initial body value from old database content
        const bodyValue = $('#body_input').val();
        if (bodyValue) {
            quill.root.innerHTML = bodyValue;
        }
        ...
        $form.on('submit', function(e) {
            ...
            $submitBtn.html('<i class="fa-solid fa-spinner animate-spin mr-1.5"></i> Memperbarui...');
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        showToast(result.message || 'Artikel berhasil diperbarui!', 'success');
                        ...
                        showToast(result.message || 'Terjadi kesalahan saat memperbarui.', 'error');
