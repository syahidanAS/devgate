...
@section('title', 'Tambah Komponen Baru — DevGate')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-800/60 pb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight flex items-center gap-3">
                <i class="fa-solid fa-boxes-stacked text-indigo-500"></i> Tambah Komponen Baru
            </h1>
            <p class="text-slate-450 text-xs mt-1">Buat produk perangkat keras, modul IoT, sensor, atau komponen elektronik lainnya ke dalam katalog toko.</p>
        </div>
        <a 
    ...
    <form action="{{ route('cms.products.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
            ...
            <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 space-y-5 backdrop-blur-sm" x-data="{ 
                specs: [
                    @if(old('specs_key'))
                        ...
                        @endforeach
                    @else
                        { key: 'Microcontroller', val: 'ESP32-D0WDQ6-V3' },
                        { key: 'Wi-Fi/Bluetooth', val: '802.11 b/g/n & BLE 4.2' },
            ...
            <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 space-y-5 backdrop-blur-sm">
                ...
                <div class="space-y-3" x-data="{ 
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-slate-800 hover:border-slate-700 bg-slate-950/40 rounded-3xl cursor-pointer transition-all">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <i class="fa-solid fa-cloud-arrow-up text-slate-500 text-2xl mb-2"></i>
                                <p class="text-[11px] text-slate-400 font-bold">Klik untuk unggah gambar produk</p>
                                <p class="text-[9px] text-slate-500 mt-1">Format PNG, JPG, JPEG (Max 2MB per gambar, disarankan rasio 1:1)</p>
                            </div>
                            <input 
                                type="file" 
                                name="images[]" 
                ...
                <div class="space-y-4 pt-2" x-show="open" x-collapse x-cloak>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea 
                            ...
                            class="w-full rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-3 text-xs text-white focus:border-indigo-500 focus:outline-none transition-all resize-none"
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        <input 
                            ...
                            min="1"
                            required
                            class="w-full rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-3 pr-12 text-xs text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none transition-all font-mono"
                        >
            ...
            <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 space-y-5 backdrop-blur-sm">
                ...
                <!-- Track Stock Toggle -->
                <div class="flex items-center justify-between bg-slate-900/30 p-3 rounded-2xl border border-slate-850" x-data="{ trackStock: {{ old('track_stock', '1') == '1' ? 'true' : 'false' }} }">
                    <div class="flex flex-col">
