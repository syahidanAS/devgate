...
@section('title', 'Edit Projek Firmware — DevGate')
...
<div class="flex items-center gap-2 text-xs font-semibold text-slate-400">
    ...
    <i class="fa-solid fa-chevron-right text-[8px]"></i>
    <span class="text-white">Edit Projek</span>
</div>
@endsection
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-800/60 pb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight flex items-center gap-3">
                <i class="fa-solid fa-pen-to-square text-indigo-500"></i> Edit Projek Firmware
            </h1>
            <p class="text-slate-400 text-xs mt-1">Perbarui detail projek firmware '{{ $firmwareProject->name }}'.</p>
        </div>
        <a 
    ...
    <div class="rounded-3xl border border-slate-800/80 bg-slate-950/45 p-6 md:p-8 shadow-xl">
        <form action="{{ route('cms.firmware-projects.update', $firmwareProject->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <input 
                    ...
                    id="name" 
                    name="name" 
                    value="{{ old('name', $firmwareProject->name) }}" 
                    required 
            ...
            <div class="space-y-2">
                ...
                >
                    <option value="" disabled>Pilih Tipe Mikrokontroler</option>
                    <option value="ESP32" {{ old('device_type', $firmwareProject->device_type) == 'ESP32' ? 'selected' : '' }}>ESP32 (Standard)</option>
                    <option value="ESP32-S3" {{ old('device_type', $firmwareProject->device_type) == 'ESP32-S3' ? 'selected' : '' }}>ESP32-S3</option>
                    <option value="ESP32-C3" {{ old('device_type', $firmwareProject->device_type) == 'ESP32-C3' ? 'selected' : '' }}>ESP32-C3</option>
                    <option value="ESP8266" {{ old('device_type', $firmwareProject->device_type) == 'ESP8266' ? 'selected' : '' }}>ESP8266</option>
                    <option value="Arduino/Other" {{ old('device_type', $firmwareProject->device_type) == 'Arduino/Other' ? 'selected' : '' }}>Arduino / Board Lainnya</option>
                </select>
                @error('device_type')
                    <p class="text-xs text-rose-450 mt-1 font-semibold">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-2">
                ...
                >{{ old('description', $firmwareProject->description) }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-450 mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
